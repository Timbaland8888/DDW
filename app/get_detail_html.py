...
class DdwBooksDetail():
    ...
    def get_field(sefl,*args,**kwargs):
        ...
        #查询详情书本的链接
        href_sql = """ select id,href,src_imag from book_info where flag is NULL """
        ...
        if len(hrefs):
            for link in hrefs:
                ...
                try:
                    ...
                    print(e)
            print('详情数据已加载到数据库。。。')
            #更新book_info 字段flag 为 ‘1’
            try:
                sefl.sql_cursor.query("update book_info set flag='1' where id in (select book_id from t_book )")
            except Exception as e:
                print(e)
        else:
            printDarkBlue('查询当当自营信息不存在或者早已插入到数据库！\n',FOREGROUND_RED)
